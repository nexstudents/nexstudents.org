...
/* ---------- colour themes ---------- */
/* 🚨 EVERY PALETTE HAS GRAPHITE'S NEUTRAL BACKGROUND. Paul, 2026-09-11, seeing
   a lesson (and the account panel over it) turn green, then maroon: "i dont
   think the background needs to be green like this. i think it should just
   be light and dark modes so white or black." So ground, surface, s2, the ink
   trio, rule, and the dark-mode control colours are IDENTICAL in all eight;
   a palette now differs only in its accents and highlight (accent*, onAccent,
   light ctl*, tick*, brass, band, word). Checked: every accent 4.5:1+ on the
   neutral backgrounds. ⚠️ Do not re-tint a ground to give a palette character. */
...
/* 🖍️ READING HIGHLIGHT COLORS - chosen per profile in the account panel's
   Settings (2026-09-11). Paul: "allow custom color also for the nexvoice
   highlight mode. so when it read the lesson it reads in that color you
   selected. it can be different than the theme color."
   Unset = the palette's own gold (--word / --band above). A pick replaces
   BOTH: --word behind the word NexVoice is reading, --band behind the sentence
   a device voice is reading. The alphas copy the palettes' own ranges.
   ✅ Contrast-checked 2026-09-11: ink over each colour, all 8 palettes, both
   modes, beats the gold it replaces (dark 4.1-4.9:1 against gold's 3.2:1).
   ⚠️ nav.js reads THIS object at build time for its swatches. Keep the
   one-line shape: key:{ name:"X", rgb:"r,g,b" }. */
var HIGHLIGHTS = {
  green:  { name:"Green",  rgb:"72,176,110" },
  blue:   { name:"Blue",   rgb:"86,160,222" },
  pink:   { name:"Pink",   rgb:"228,118,166" },
  orange: { name:"Orange", rgb:"232,146,62" },
  purple: { name:"Purple", rgb:"160,130,224" }
};
var HL_ALPHA = { light:{ word:.66, band:.36 }, dark:{ word:.58, band:.28 } };
...
function applyTheme(){
  ...
  /* The profile's Reading Highlight, if one is picked, over the palette's gold.
     Read fresh each time so a change in the panel lands on the next repaint. */
  /* 🚨 GUARDED, AND IT HAS TO BE. On a maths, integers or English page this
     function is spliced in by voice-player.js AFTER that page's own code, which
     calls applyTheme() before these two vars are assigned (a var is hoisted
     but empty until its line runs). Unguarded, HIGHLIGHTS[...] threw and the
     whole lesson engine died: no problems drawn at all. Found 2026-09-11 by
     actually doing a maths lesson, which Paul asked for. The later calls run
     after the vars exist, so the highlight still lands. */
  var hl = (typeof HIGHLIGHTS === "object" && HIGHLIGHTS) ? HIGHLIGHTS[load("highlight", "")] : null;
  if (hl && typeof HL_ALPHA === "object" && HL_ALPHA){
    var al = HL_ALPHA[currentMode()];
    root.style.setProperty("--word", "rgba(" + hl.rgb + "," + al.word + ")");
    root.style.setProperty("--band", "rgba(" + hl.rgb + "," + al.band + ")");
  }
...
applyTheme();
/* The account panel changed a reading setting (lesson colors or the reading
   highlight) while this lesson is open. This engine keeps its names private,
   so the panel cannot call applyTheme; it fires "ns:settings" and the lesson
   re-reads its own keys. Found 2026-09-11: the change only showed on reload. */
document.addEventListener("ns:settings", function(){
  var k = load("theme", "graphite");
  themeKey = THEMES[k] ? k : "graphite";
  applyTheme();
});
  ...
  fetch("voice.json", { cache: "no-cache" })
    ...
    .then(function(j){
      ...
      if (j.textHash && TEXT_HASH && j.textHash !== TEXT_HASH){
        AUDIO = null;
        /* 🚨 SAY WHICH OF THE TWO THINGS HAPPENED. The picker's fallback text
           was "NexVoice - not available" for BOTH "never recorded" and this,
           and they need opposite actions: one is us owing a bake, the other is
           a reader holding a cached page. Paul, 2026-09-15: "it says NexVoice
           is not available on mobile" while the PC was fine on the same lesson
           - a stale page on the phone is exactly what this branch catches, and
           the message sent us looking in the wrong place. */
        window.__nsVoiceStale = true;
...
/* 🚨 A FALLBACK THAT SAYS NOTHING IS HOW A WRONG VOICE LOOKS LIKE A WORKING ONE.
   Two paths drop from NexVoice to the device voice: the clip fails to play
   (offline, a slow R2, a mobile-data hiccup), or we reach speak() still
   believing we are baked when AUDIO never arrived. Both used to happen in
   silence with the picker still reading NexVoice.

   Paul, 2026-09-15, from his phone: "it does show it is available but the voice
   is not correct and is defaulting to another voice I think from my phone ...
   it sounds like a generic ai female voice." He could hear it was wrong and the
   page insisted it was right, which cost an evening of looking in the wrong
   place.

   ⚠️ Whatever else changes here, the picker must never name a voice that is not
   the one speaking.

   ⚠️ selectValue lives inside the picker's closure and is NOT reachable from
   here. window.syncVoiceUI is already exported for exactly this job: it moves
   the <select> to the device voice once engine is "device". Then nsRenderVsel
   repaints the list the reader is actually looking at. */
function dropToDeviceVoice(){
  if (engine === "device") return;
  engine = "device";
  if (typeof window.syncVoiceUI === "function") window.syncVoiceUI();
  if (typeof window.nsRenderVsel === "function") window.nsRenderVsel();
}
...
if (!supported && !hasStudio() && !hasKey()){
  ...
  function loadVoices(){
    ...
    if (AUDIO_TRACKS.length){
      ...
      nv.value = "__studio__";
      nv.textContent = window.__nsVoiceStale
        ? "NexVoice — refresh this page"
        : "NexVoice — not recorded yet";
      nv.disabled = true;
    ...
    if (!want || !selectValue(want)){
      ...
      /* ⚠️ The second branch here used to repeat the first one's condition
         exactly, so it could never run. Collapsed rather than guessed at. */
  ...
  function applyVoiceChoice(val, persist){
    ...
    if (val.indexOf("g:") === 0){
      ...
      var id = val.split(":")[1];
      /* 🚨 ONLY CLAIM "baked" IF A TRACK WAS ACTUALLY FOUND. useTrack returns
         false for a reason and this line used to throw it away, so engine was
         set to "baked" with AUDIO still null and speak() fell through to
         playSynth - the device voice - while the picker went on saying
         NexVoice. Paul, 2026-09-15, from his phone: "it does show it is
         available but the voice is not correct and is defaulting to another
         voice I think from my phone ... it sounds like a generic ai female
         voice."
         🚨 THE PLACEHOLDER IS THE COMMON CASE, not a saved pick. Until
         voice.json resolves, the only NexVoice option is the DISABLED one whose
         value is the bare "__studio__", with no id after the colon. That still
         matched this branch, so every lesson claimed baked audio for as long as
         the fetch took. On a desktop that is imperceptible; on a phone on
         mobile data it is long enough to press play. */
      if (id && useTrack(id)) engine = "baked";
      else if (AUDIO_TRACKS.length && useTrack(AUDIO_TRACKS[0].id)) engine = "baked";
      else engine = "device";
    } else {
      engine = "device";
...
function playClipFrom(mine, clip){
  ...
  if (p && p.then){
    p.then(function(){ rafId = requestAnimationFrame(tick); })
     .catch(function(){ if (mine === gen) { dropToDeviceVoice(); playSynth(mine); } });
  } else {
    rafId = requestAnimationFrame(tick);
  }
}
...
function speak(i){
  ...
  if (engine === "google" && hasKey()) playGoogle(mine);
    ...
    /* 🚨 NEVER PLAY ONE VOICE WHILE THE PICKER NAMES ANOTHER. This used to fall
       straight through to the device voice with the label still reading
       NexVoice, which is how a wrong voice looks like a working one. If we get
       here still believing we are baked, the belief is wrong: say so in the UI
       by actually moving the picker to the device voice that is about to
       speak. */
    if (engine === "baked") dropToDeviceVoice();
    playSynth(mine);
  }
}
...
/* Bring back the answers he already gave, so coming back to a lesson shows the
   page he left rather than a blank one. Runs right after the cards are built.
   ⚠️ EVERY INDEX IS RANGE-CHECKED against the lesson as it stands today. A
   rebuilt lesson can have fewer questions or fewer choices than the record was
   written against, and a stale pick must be dropped rather than painted onto
   whatever now sits at that index. Nothing else is assumed about the record:
   an old one with no `a` simply restores nothing, exactly as before. */
(function restoreAnswers(){
  var rec = null;
  try { rec = JSON.parse(load("prog:" + LESSON_ID, "null")); } catch (e){}
  if (!rec || !rec.a || !rec.a.length) return;
  var any = false;
  rec.a.forEach(function(ci, i){
    var Q = QUESTIONS[i];
    if (ci === null || ci === undefined || !Q || !Q.choices) return;
    if (ci < 0 || ci >= Q.choices.length) return;
    answered[i] = ci;
    paintAnswer(i, ci);
    any = true;
  });
  (rec.h || []).forEach(function(v, i){ if (v && QUESTIONS[i]) hunted[i] = true; });
  if (typeof rec.ff === "number") foundFirstTry = rec.ff;
  if (!any) return;
  paintProgress();
  /* ⚠️ A day with NO questions has done === total === 0, which would show a
     score on a lesson nobody has touched. Count the day only if it has any. */
  var d1 = dayStats(1), d2 = dayStats(2);
  if ((d1.total && d1.done === d1.total) || (d2.total && d2.done === d2.total)) showScore();
})();
...
/* 🚨 PAINTING AN ANSWER IS SPLIT OUT OF GIVING ONE, so restoring a saved answer
   draws EXACTLY what answering it drew. Two painters is how a reloaded page
   starts looking different from the page the student left. */
function paintAnswer(qi, ci){
  var Q = QUESTIONS[qi];
...
/* A light progress record on every answer, so the lesson card on the shelf knows
   how far in he is without the lesson having to be finished first.

   🚨 IT CARRIES THE ANSWERS THEMSELVES NOW. They used to live only in the
   `answered` array, in memory, so a reload - or a phone dropping the tab out of
   memory - wiped every answer while the shelf card still read "4 of 8 answered".
   The student came back to a blank page with no way to see what he had done.
   ⚠️ nsProgPush keeps `detail` VERBATIM, so writing them HERE is also what sends
   them to the account: a parent can see from another device what has been
   answered, and a cleared cache comes back on the next sign-in. Adding a field
   to this record needs no change in nav.js. */
function saveProg(){
  var d1p = dayStats(1), d2p = dayStats(2);
  var picks = [], hunts = [];
  QUESTIONS.forEach(function(Q, i){
    picks[i] = answered[i] === undefined ? null : answered[i];
    hunts[i] = !!hunted[i];
  });
  store("prog:" + LESSON_ID, JSON.stringify({
    ...
    a: picks, h: hunts, ff: foundFirstTry
  }));
}
