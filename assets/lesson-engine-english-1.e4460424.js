...
PARTS.forEach(function(part){
  ...
  part.s.forEach(function(text){
    ...
    text.split(" ").forEach(function(w, k){
      ...
      ws.className = "w";
      ws.textContent = w;
      if (glossAt.indexOf(k) !== -1) ws.className = "w gloss";
      span.appendChild(ws);
...
(function inlineGlossary(){
  ...
  function closeBox(){
    ...
    [].forEach.call(story.querySelectorAll(".w.gloss.opened"), function(w){
  ...
  story.addEventListener("click", function(ev){
    ...
    var hit = ev.target.closest(".w.gloss");
    if (!hit) return;

    /* gather the whole marked run so "cell membrane" reads as one term */
    var sent = hit.closest(".sent");
    var all = [].slice.call(sent.querySelectorAll(".w"));
    var at = all.indexOf(hit), run = [hit];
    for (var a = at - 1; a >= 0 && all[a].classList.contains("gloss"); a--) run.unshift(all[a]);
    for (var b = at + 1; b < all.length && all[b].classList.contains("gloss"); b++) run.push(all[b]);
  ...
  [].forEach.call(story.querySelectorAll(".w.gloss"), function(w){
    w.setAttribute("tabindex", "0");
    w.setAttribute("role", "button");
  });
  story.addEventListener("keydown", function(ev){
    if (ev.key !== "Enter" && ev.key !== " ") return;
    if (!ev.target.closest || !ev.target.closest(".w.gloss")) return;
    ev.preventDefault();
    ev.target.closest(".w.gloss").click();
  });
})();
