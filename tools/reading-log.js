...
(function(){
  ...
  /* 🚨 elapsed IS CLAMPED TO THE TARGET, AND THIS IS A REAL BUG FIX, NOT TIDYING.
     Paul, 2026-09-03, from his phone: "the timer keeps resetting me back at a 2 hour
     time limit, it doesn't play the sound after the clock runs down and does not show
     the bell icon, and last instead of counting down it counts back up."
     All three were ONE fault. seekFromEvent() sets elapsed straight from the finger,
     so tapping the LEFT end of the scrub - easy to do by accident while scrolling a
     phone - set elapsed to the whole target while ran stayed 0. Press play and it
     rebased from there: 3 seconds read, elapsed 2 hours 3 seconds, so left went
     negative and the clock counted UP from the first tick.
     And it could never recover. The bell watches ran, correctly, so it would have
     needed two real hours to fire - which means no sound, no bell icon, and Save
     locked forever, because rangOut is raised only by ring().
     ⚠️ DO NOT "FIX" THIS BY LETTING THE DRAG RAISE ran. That is the exact hole the
     two-number split was built to close - a child could then drag to the end and log
     a half hour he never read. elapsed is display; ran is evidence. Clamping the
     DISPLAY is the fix; the evidence is left alone. */
  function clampElapsed(){
    var total = target * 60;
    if (elapsed < 0) elapsed = 0;
    if (elapsed > total) elapsed = total;
  }
  function rebase(){ clampElapsed(); runStart = Date.now(); ranBase = ran; elapsedBase = elapsed; }
  function sync(){
    if (!running) return;
  ...
  /* 🚨 THE BELL WAS SILENT, AND IT WAS THE AUDIO CONTEXT BEING ASLEEP.
     Paul, 2026-09-03: "the timer works on the local host timer sound but not the
     website", then "actually it doesnt now on the local" - so it was never the two
     pages differing. They are byte for byte identical; it is the CONTEXT.
     A context suspends when the tab is hidden or the phone screen goes off, and this
     page is MEANT to be put down while a paper book is read - the alarm's whole job
     happens at the moment it is least likely to be awake.
     🚨 AND resume() IS ASYNCHRONOUS, which is what made the first attempt at this
     useless. Calling resume() and then scheduling against actx.currentTime schedules
     against a clock that has not restarted, so every note lands in the past and is
     dropped without an error. The notes have to be scheduled AFTER the resume
     settles. That is the whole bug.
     ⚠️ A 0.06s lead on every note, never currentTime exactly. A note scheduled at
     "now" is already marginally late by the time the graph is built, and some
     browsers drop it rather than play it late. */
  function notes(){
    if (!actx) return;
    var now = actx.currentTime + 0.06;
  ...
  function chime(){
    /* ⚠️ VIBRATE TOO, AND FIRST. Audio can be refused outright - a silent phone, a
       muted tab, a browser that will not resume without a fresh tap - and a bell
       nobody can hear is the bug being reported. Vibration has none of those
       conditions. Unsupported on desktop and on iOS, where it is simply ignored. */
    try { if (navigator.vibrate) navigator.vibrate([180, 90, 180]); } catch(e){}
    try {
      if (!actx) return;
      if (actx.state === "running") { notes(); return; }
      var p = actx.resume();
      if (p && p.then) { p.then(notes, notes); } else { notes(); }
    } catch(e){}
  }
  ...
  function paint(){
    ...
    /* ⚠️ This line reports ran, never elapsed. Dragging the scrub moves
       ...
       🚨 AND IT SAYS SO WHEN THE TWO DISAGREE. With elapsed clamped, dragging the
       bar to the end parks the clock at 0:00 while the bell still waits on ran -
       correct, but silent, and a timer that reaches zero and does nothing reads as
       broken. That silence is what Paul hit: "it doesn't play the sound after the
       clock runs down and does not show the bell icon." So when the bar is spent
       and the reading is not, the page explains itself instead of just sitting. */
    var short = target * 60 - ran;
    elElapsed.textContent = ran === 0
      ? (running ? "Starting" : "Not started")
      : "Read for " + spell(ran) +
        (left <= 0 && short > 0 ? " - the bar was moved, so there is still " + spell(short) + " to read" : "");
  ...
  /* 🚨 EVERY TARGET IS REMEMBERED NOW, not only one over 30 minutes.
     It used to store a longer target and treat a shorter one as a one-off, which
     read as the box resetting itself. Paul, 2026-09-03: "the timer keeps resetting
     me back at a 2 hour time limit." It was doing exactly what it was written to do
     - he had once set 120, then set something shorter, and the shorter one was
     thrown away on reload - and the rule was simply wrong. The box should say what
     he last set.
     ⚠️ input as well as change. On Android a number field often does not fire
     change until it loses focus, so typing 15 and going straight to Play left the
     target at the old value while the box showed the new one - the timer and the
     screen disagreeing about the same number. */
  function applyTarget(store){
    var n = parseInt(elTarget.value, 10);
    if (!n || n < 1) return;                 /* mid-typing, not yet a number */
    if (n > 600) { n = 600; elTarget.value = n; }
    target = n; buildTicks();
    if (store) saveTarget(n);
    clampElapsed();
    rebase();
    paint();
  }
  elTarget.addEventListener("input", function(){ applyTarget(false); });
  elTarget.addEventListener("change", function(){
    var n = parseInt(elTarget.value, 10);
    if (!n || n < 1) { elTarget.value = target; return; }   /* left empty - put it back */
    applyTarget(true);
  });
