...
const UPPER = {
  ...
  /* Cap J takes a bar across the top - Paul, 2026-08-27, confirmed off the
     reference. Two strokes: the stem and hook down, then the bar left to right,
     which is the order and the direction the reference numbers them. */
  /* 🚨 J is laid out from its STEM, not from the cl/cr rails, and its HOOK is a
     HALF circle, not a quarter turn. Paul, 2026-08-27: "notice the hook." In the
     reference the tail sweeps left around the baseline and RISES BACK UP,
     finishing level with where the stem stopped curving and pointing upward.
     A quarter turn stops at the baseline pointing left, which reads as a letter
     that got cut off.

     The bar is centred on the stem. The stem sits right of centre because the
     hook swings left, so a bar drawn rail to rail overhangs hard on the left and
     barely at all on the right - which is what Paul caught: "it needs to be
     centered on the J." Stem 84, bar 24 either side, hook reaching 30, so the
     whole letter sits square in its cell.
     ⚠️ The reference itself hangs the bar to the LEFT of the stem rather than
     centring it. Centred is what Paul asked for - deliberate, not an oversight. */
  J: [S(line([84, TOP], [84, 100]), arc(57, 100, 27, 30, 0, 180)),
      S(line([60, TOP], [108, TOP]))],
...
const DEFS = `<svg width="0" height="0" style="position:absolute" aria-hidden="true"><defs>
  <!-- 🚨 A POINTED dart, not a squat triangle. Paul, 2026-08-27: "make the ends
       of the arrows look a little bit more pointy ... I can't really tell even
       zooming in." The head was 6.5 long by 6.5 tall - as wide as it was long,
       so at worksheet size it read as a dot with no direction in it. 15 long by
       7 tall is better than twice as long as it is wide, and the tail is swept
       BACK to a notch (the 3.5,3.5 waypoints) so the whole shape leans forward.
       It still fits inside the 11-unit letter stroke it sits in. -->
  <marker id="ahead" viewBox="0 0 15 7" refX="14" refY="3.5" markerUnits="userSpaceOnUse"
          markerWidth="15" markerHeight="7" orient="auto">
    <path d="M0,0 L15,3.5 L0,7 L3.5,3.5 z" fill="#14181d"/>
  </marker>
  <marker id="ahead-i" viewBox="0 0 15 7" refX="14" refY="3.5" markerUnits="userSpaceOnUse"
          markerWidth="15" markerHeight="7" orient="auto">
    <path d="M0,0 L15,3.5 L0,7 L3.5,3.5 z" fill="#fff"/>
  </marker></defs></svg>`;
...
/* 🚨 TWO cells per letter, not three, and several letters to a row.
   ...
   A letter is a GROUP of four cells - capital and lowercase carrying the arrows,
...
/* A ruled page with nothing on it. Paul, 2026-08-27: "add a blank lined sheet
   after the wall chart ... we can call it a practice sheet." Same ruling as the
   letters above it - top line, dashed midline, baseline, descender - so a child
   moves from tracing to writing without the paper changing under them. Cells
   sit edge to edge, which makes each row read as one continuous rule. */
const blankCell = () => `<svg viewBox="0 0 ${W} ${H}" class="cell">${rules()}</svg>`;
const practiceRows = (rows = 7, cells = 12) =>
  Array.from({ length: rows }, () =>
    `<div class="row">${Array.from({ length: cells }, blankCell).join("")}</div>`).join("\n");
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .guide{fill:none;stroke:var(--ink);stroke-width:1.7;stroke-linecap:round}
  .guide.on-ink{stroke:#fff;stroke-width:1.9}
...
<h2>Worksheet &mdash; A to Z, capital and lowercase</h2>
<div class="sheet">
${wsRows()}
</div>

<h2>Print this one for the fridge</h2>
...
/* 🚨 DUAL MODE. Run directly it writes the standalone preview, which is what
   the review loop uses. REQUIRED, it hands the letter geometry and the sheet
   markup to build-worksheets.js, so the live worksheet and the preview are the
   same 52 letterforms and can never drift apart. The letter CSS travels with
   the markup rather than living in worksheet.css, which also keeps make-cover
   honest - a screenshot of the page picks up inline rules automatically. */
const LETTER_CSS = `
  /* 🚨 ON SCREEN the letters follow the SITE THEME; ON PAPER they are forced
     back to ink on white. Hardcoding near-black ink made every model letter
     invisible in dark mode, which is the site default - the page showed the
     faded tracing pair and an empty space where the demonstration should be.
     The model is --fg, the tracing copies are the same colour at low opacity,
     and the knocked-out arrow is --bg, so all three follow whichever theme the
  ...
  .hw{--rule:var(--line)}
  /* 🚨 FLUID, NOT FIXED. Paul, 2026-08-27, sent a phone screenshot: the letter
     rows ran straight off the right edge of the white sheet and over the page
     background. They were 12 cells of 57px - 704px - with nothing responsive
     about them, so on a 360px phone two thirds of every row hung outside its
     own paper.
     Cells now SHARE the row instead of claiming a width: each group takes an
     equal share, each cell an equal share of that, and aspect-ratio holds the
     letter proportions while the SVG scales itself to whatever box it lands in.
     min-width:0 is what actually lets a flex child shrink below its content -
     without it they refuse to give ground and overflow regardless. */
  .hw .cell{flex:1 1 0;min-width:0;width:auto;height:auto;
    aspect-ratio:${W} / ${H};overflow:visible}
  .hw .row{display:flex;align-items:flex-end;margin-bottom:7px;gap:10px}
  .hw .grp{display:flex;flex:1 1 0;min-width:0}
  .hw .rule{stroke:var(--rule);stroke-width:1.4;fill:none}
  .hw .rule.solid{opacity:.72} .hw .rule.dash{stroke-dasharray:7 7;opacity:.55}
  .hw .rule.faint{opacity:.26}
  .hw .ink{fill:none;stroke:var(--fg);opacity:.30;stroke-width:11;
    stroke-linecap:round;stroke-linejoin:round}
  .hw .ink.solid{opacity:1}
  .hw .ink.dt{fill:var(--fg);stroke:none}
  .hw .ink.ghost{opacity:.15}
  .hw .guide{fill:none;stroke:var(--fg);stroke-width:1.7;stroke-linecap:round}
  .hw .guide.on-ink{stroke:var(--bg);stroke-width:1.9}
  .hw .grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px 22px}
  .hw .two{display:flex;min-width:0}
  .hwh{font-size:15px;margin:26px 0 6px}
  @media print{
    /* Paper is white whatever the reader had on screen, so the theme colours
       are overridden outright rather than hoped about. */
    .hw .ink{stroke:#111;opacity:.28}
    .hw .ink.solid{stroke:#111;opacity:1}
    .hw .ink.dt{fill:#111}
    .hw .ink.ghost{opacity:.14}
    .hw .guide{stroke:#111}
    .hw .guide.on-ink{stroke:#fff}
    ...
    /* 🚨 THREE SHEETS, THREE PAGES. Paul, 2026-08-27: "this will be the entire
       ...
       section starts its own printed page - otherwise the wall chart lands
       halfway down page 1 and "print page 2" gets a parent half of each.
       .hwh sits only on the second and third headings, so this breaks before
