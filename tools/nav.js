...
function adEdit(H,row,kindIn){
  ...
  H.body.innerHTML=
    ...
    /* 📈 The arrow into their Progress (Paul: "have a arrow for progress and
       you arrow over"). A saved student only - a new one has none yet. */
...
/* ── 📈 THE PROGRESS VIEWS (2026-09-11, step 2) ────────────────────────────
   ...
   their overall progress ... have a arrow for progress and you arrow over and
   it shows course progress and overall score and what unit they are on
   next. the student needs something similar but much easier or simple. the
   parent also needs a way to reset the students progress and give a warning",
   and "the parent needs the ability to reset test."
   Courses come from /assets/courses.json (written by build-pages from the
   same outlines the shelves page through). Progress comes from the progress
   table, plus this device's unsent records when that student is the one on.
   Points: 10 per finished lesson, +5 when the score is 90% or better (Paul's
   pick). Maths, integers and English are retry-until-right and carry no
   score, so they add to "finished" and points but never to the average. */
var adCourses=null;
...
/* Which courses a student sees: the ones for their grade, plus any course
   they have progress in. No grade and no progress -> every course. */
...
/* 🧹 Forget a student's records ON THIS DEVICE too (their live copy if they
   are on, their stash, the pushed snapshot), or a reset reappears on the next
   push. lessonId null = every lesson. */
...
/* THE PARENT'S PROGRESS VIEW for one student. */
...
/* RESET ALL: its own warning screen, the same shape as Remove Student. */
...
/* THE STUDENT'S OWN, SIMPLE VERSION (inside My Profile): three numbers and
   what is next in each of their courses, each one a link into the lesson. */
...
function adProfile(H){
  ...
  var md=(adUser&&adUser.user_metadata)||{};
  /* 🔑 ACCESS TYPE, COMING SOON. Paul, 2026-09-11: "the parent also needs what
     account access type they have ... once you paid its part of all the
     accounts." It belongs to the ACCOUNT (both parents, every student) and
     will read the owned scopes of ROADMAP 42/53. Every account is Free today,
     which is true, so the line says so instead of inventing a tier. */
  H.body.innerHTML=
...
function adLoad(){
  ...
  /* ⚡ ONE REQUEST, ONE REPAINT (migration 020). This was three calls, each
     repainting as it landed, which is what made the tiles arrive in stages.
     NSAccount.bootstrap() returns the user, the profiles and the PIN map
     together, and falls back to the old three if the function is not deployed
     yet - so this is safe to ship before the SQL is applied. */
