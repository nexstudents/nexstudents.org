...
const { comicsPage, SERIES } = require('./pages/comics');

const { privacyBody } = require('./pages/privacy');

const { termsBody } = require('./pages/terms');

const { refundBody } = require('./pages/refund');
...
(function(){
  ...
  /* 🚨 THE PAGE JUMPED TO THE TOP WHEN THE WINDOW OPENED. Paul, 2026-09-11:
     "I opened it and it did move the page so make sure to fix that." The whole
     point of this window is that nothing behind it moves, so this was the same
     complaint the <details> caused, wearing a different hat.
     Two separate movements, and both need handling:
       1. showModal() scrolls, because it moves focus into a dialog that sits at
          the end of <body>. keptY is read BEFORE the call and restored after.
       2. Hiding the scrollbar reflows the page narrower, which slides the whole
          layout sideways under the backdrop. paddingRight replaces exactly the
          width the scrollbar was taking.
     ⚠️ overflow:hidden on <html> KEEPS scrollTop, which is why the page is not
     also pinned with position:fixed - that technique zeroes the scroll and needs
     restoring, and it breaks position:sticky on the nav. */
  var keptY = 0, keptPad = "";
  function lock(){
    keptY = window.scrollY;
    var bar = window.innerWidth - document.documentElement.clientWidth;
    keptPad = document.documentElement.style.paddingRight;
...
(function(){
  ...
  /* 🚨 A LESSON IS FINISHED IN FOUR DIFFERENT SHAPES, AND THE CARD MUST KNOW ALL
     OF THEM. This only read "ns:done:", which ONLY the history/science template
     writes — so every finished maths, integers and English lesson showed as
     unfinished, forever. Worse, the "is-part" branch below returns early, so
     maths/long-division sat at "5 of 5 answered" with a half progress bar while
     its own record said complete:true. Found 2026-09-08 on Kolten's real progress.
       ns:done:<id>  {complete:true, score, total}   history, science
       ns:prog:<id>  {done, total, complete}          maths, integers, English
       ns:prog:<id>  {done, total, d1, d1t, ...}      history/science part-progress
     ⚠️ Never decide this from done > 0 alone — that is "started", not "finished".
     ⚠️ NO BACKTICKS IN THIS COMMENT. progressScript is a template literal and a
     backtick here closes it; the build then dies on the next word. It has now cost
     three builds: 2026-08-31, and again writing this very comment. */
  function finished(d, pr){
    if (d && d.complete !== false) return true;
    if (!pr) return false;
    if (pr.complete === true) return true;
    return typeof pr.done === "number" && typeof pr.total === "number" &&
           pr.total > 0 && pr.done >= pr.total;
  }
  function isDone(id){ return finished(read("ns:done:", id), read("ns:prog:", id)); }
  ...
  cards.forEach(function(c){
    ...
    if (finished(d, pr)){
      ...
      /* 🚨 THE TWO LESSON TYPES MEAN DIFFERENT THINGS BY A PERCENTAGE, and Paul
         copies this straight into HomeschoolGrades, so it must not flatter.
         History and science ask each question ONCE, so d.pct is a real mark.
         Maths, integers and English are retry-until-right by design (Paul's rule:
         no punishment for wrong), so there is no partial score to report —
    ...
    if (pr && pr.done > 0){
      ...
      /* 🚨 PAUL'S EXACT FORMAT, 2026-09-11: "like 15/16 Completed  95%". Count
         first, then the word, then the percentage — the same count-AND-percentage
         rule the quizzes follow → [[feedback-quiz-percentage]]. It read
      ...
      /* 🚨 NO GREEN BAR. Paul, same message: "you can get rid of the progress green
         ...
         second element drawn into .cbody that changed the card's height as soon as
         a lesson was started. */
      return;
    }
...
const ssStateBody = (st) => `<div class="band"><div class="wrap">
    ...
    ${ssGradeLinks(st, null)}` : `<div class="ss-empty"><b>No subject information for ${st.name} yet.</b>
      <span>We are adding states one at a time.</span></div>`}
    ${ssMore(st, false)}
    </div>
  </div></div>`;
...
const SOON_PAGES = [
  ...
  <!-- ⚠️ NO "DOWNLOADS ON THIS DEVICE" BOX. Paul, 2026-09-10: "we are just going
       to do order history page instead like the lizzie one." Orders live in the
       signed-in Orders row; a guest reaches an order from its receipt email. -->
...
(function(){
  ...
  /* 🔊 THE STARTUP SOUND, ON THE TAP THAT SIGNS IN.
     Paul asked for it "when the login screen appears", then agreed to hang it
     on the tap instead: "could we wire it for after a tap for login". That is
     not a compromise, it is the only version that works. A phone refuses audio
     that no gesture asked for, so a sound fired when a screen appears is
     silently dropped - the same rule that had NexVoice reading in the device
     voice from sentence two.
     ⚠️ It must start INSIDE this handler, before any await or fetch, or the
...
<\/script>` },
  ...
  <div id="cartFull" class="hidden">
    <div class="ck-grid">
      ...
      <aside class="ck-sum">
        <!-- Small bits from the Lizzie Peirce checkout, 2026-09-10: Subtotal,
             Tax, a rule, Total; a Your Account line when signed in; and the
             secure checkout line under the button. Paul: "take small bits". -->
        <h2 class="ck-sum-h">Order Summary</h2>
        <div class="ck-sum-row"><span>Subtotal</span><span id="cartTotal">&mdash;</span></div>
        <div class="ck-sub"><span>Tax</span><span>$0.00</span></div>
        <hr>
        <div class="ck-sum-row ck-total"><span>Total</span><span id="cartGrand">&mdash;</span></div>
        <hr>
        <div class="ck-who hidden" id="coWho">
          <div class="ck-who-top"><b>Your Account</b>
            <button type="button" class="ck-out" id="coOut">Sign Out</button></div>
          <span class="ck-note" id="coWhoEmail" style="margin:4px 0 0"></span>
        </div>
        <form id="coForm">
          <label class="ck-note" for="coEmail" id="coLabel" style="margin:0">
        ...
        <p class="ck-note ck-center" id="coSignIn">Already have an account?
          <a href="/account/">Sign in here</a></p>
        <p class="ck-secure"><svg viewBox="0 0 20 20" width="16" height="16" aria-hidden="true"
          fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"
          stroke-linejoin="round"><path d="M10 2.5 16 5v4.6c0 3.7-2.6 6.6-6 7.9-3.4-1.3-6-4.2-6-7.9V5z"/>
          <path d="m7.2 10 2 2 3.6-3.8"/></svg>Secure SSL Checkout</p>
      </aside>
...
(function(){
  ...
  function paint(){
    ...
    NSAccount.priceList(items).then(function(rows){
      ...
      $("cartGrand").textContent = money(total);
  ...
  function prefill(){
    ...
    NSAccount.getUser().then(function(u){
      if(!(u && u.email)) return;
      /* Signed in: a Your Account line like the reference, not an email box.
         The input stays in the form, filled and hidden, so submit is unchanged. */
      $("coEmail").value = u.email;
      $("coWhoEmail").textContent = u.email;
      $("coWho").classList.remove("hidden");
      ["coLabel", "coEmail", "coSignIn"].forEach(function(id){ $(id).classList.add("hidden"); });
    });
    $("coOut").onclick = function(){ NSAccount.signOut(); location.reload(); };
  }
